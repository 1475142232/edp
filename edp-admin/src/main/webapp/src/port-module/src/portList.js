KISSY.add("port-module/src/portList",function(S,IO, Cookie){
	function PortList(){
		var _this = this;
	    ...
	    this.getPortList = function(){
			...
			if(userRole === "3" && userBool){
				$("#portList").html('<a id="addScene" href="javascript:;" data-toggle="modal">+</a>');
			}else{
				$("#portList").html('<a id="addScene" href="javascript:;" data-toggle="modal" style="display:none;">+</a>');
			}
			//获取项目列表
			$.ajax({
	             type: "POST",
     				...
     				if(userRole === "3" && userBool){
	                     ...
	                     //删除
	             		 $(".togglelist2").click(function(){
	             			$("#dl_" + this.id).toggle();
	             			 _this.delPort(this.id);
	             		 });
	             		 //编辑
	             		 $(".togglelist").click(function(){
	             			$("#dl_" + this.id).toggle();
	             			_this.editPort(this.id);
	             		 });
     				}
                  }
        	});
