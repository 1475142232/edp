...
//                }
	...
	@Override
	public List<List<TaskInfoDto>> findTaskInfoByUserId(Integer userId,String role) {
	...
	@Override
	public List<TaskInfoDto> findTaskInfoByProductId(String productId) {
		TaskInfoPoCriteria example = new TaskInfoPoCriteria();
        TaskInfoPoCriteria.Criteria criteria = example.createCriteria();
        criteria.andProductIdEqualTo(productId);
        criteria.andTaskStateEqualTo("02");
	...
	@Override
	public List<List<TaskInfoDto>> findTaskInfoByProductIdAndDate(String productId, String date) {
		List<List<TaskInfoDto>> taskInfoDtoes=new ArrayList<List<TaskInfoDto>>();
        String[] state = {"01,04","02"};
        for(int i=0;i<2;i++){
        	List<TaskInfoPo> po = taskInfoPoMapper.findTaskInfoByProductIdAndDate(date,state[i],productId);
        	List<TaskInfoDto> taskInfoDto=BeanCopyUtils.populateTListbyDListBySpring(po,TaskInfoDto.class);
        	taskInfoDtoes.add(taskInfoDto);
        }
		return taskInfoDtoes;
	...
	@Override
	public List<String> findFunctionIdByScene(String id) {
        ...
        List<TreeInfoPo> tree = treeMapper.selectByExample(example);
        String sceneId="";
        for(int i = 0 ; i<tree.size() ; i++){
        	sceneId+=tree.get(i).getId();
        	if(i!=tree.size()-1){
        		sceneId+="";
        	}
        }
		return taskInfoPoMapper.findFunctionIdByScene(sceneId);
	}


	@Override
	public Integer editTaskState(List<String> ids) {
        TaskInfoPo po = new TaskInfoPo();
        po.setTaskState("03");
        TaskInfoPoCriteria example = new TaskInfoPoCriteria();
        TaskInfoPoCriteria.Criteria criteria = example.createCriteria();
        criteria.andIdIn(ids);
		return taskInfoPoMapper.updateByExampleSelective(po, example);
	}
