...
ORYX.CONFIG.CANVAS_HEIGHT =				10000;
			...
			this._jsonStencil.propertyPackages.each((function(ppId) {
				...
				if(pp) {
					pp.each((function(prop){
						...
						if(oProp.id() !== "overrideid" && oProp.id() !== "belongtofunction" && type === "7"){
							this._properties[oProp.prefix() + "-" + oProp.id()] = oProp;
						}else if(oProp.id() !== "overrideid" && oProp.id() !== "belongtotable" && type === "6"){
							if(oProp.id() !== "ifrelevance"){
								this._properties[oProp.prefix() + "-" + oProp.id()] = oProp;
							}
						}
					}).bind(this));
...
function init() {
    ...
    // Else fetch the model from server and display editor
    	...
    	var reg = new RegExp("(^|&)modelId=([^&]*)(&|$)");
        var r = window.location.search.substr(1).match(reg);
		var modelId = r != null ? unescape(r[2]) : "";
